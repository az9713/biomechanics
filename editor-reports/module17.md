...
| B6d (§7 code, B12a) | after 244 | **New** §7 code block: computes the impact speed, impact force, critical density and the hip-protector case. Capstone VI had no code. | Run from the edited file: `critical density = 79.1% of young` and `with a hip protector (F/sqrt2): 66.6%`, matching its comments. pycodestyle clean. |
...
| B11c | 343-344 (K3) | K3's **statement and solution together** (one anchored replacement over both lines). The statement asked for an optimisation over a trunk lean that Model 3.1 does not contain, so the problem was ill-posed; it now asks the reader to build the minimal extension first, and $\phi$ is declared local to the problem (B9). The solution then derives the closed form $\sin\phi^*=d_0/(2\ell_T)$, gives $\phi^*=19.5^\circ$, says the extension is quasi-static and not the full momentum strategy, and carries a copy-buttoned code block. | Shipped block run: `optimum : phi = 19.5 deg, knee 47.0, hip 47.2 N m`, `closed form: phi* = asin(d0/(2*l_T)) = 19.5 deg`, `worst-joint torque falls 50%`. Grid optimum and closed form agree to 0.03 degrees. |
| B11d | 347 (K4) | K4's solution rewritten and a Monte Carlo code block added - the row this table previously omitted. The solution now reports the mean, the one-sigma spread and the 95 percent interval as three different things, names which is which, and reads the error budget off the dominant parameters. | Shipped block run: `mean 108.6 N m, 1 sigma 15.3 N m (14.1%)` and `95% interval 80 to 140 N m (109 +/- 27%)`, matching its `# ->` comments. This interval is the one S4-S5 and B4d quote. |
...
| B7 | after 362 | The Appendix gains a **notation table** (19 rows, 20 after B9i: each symbol with units and first-use section, and both symbol collisions from B9 recorded explicitly) and a **parameter table** (24 rows: every number classed as *assumed*, *derived*, or *Module N*). The module's own template element (6) demands one and `prompt.txt:908` requires one per capstone; there was none. | Every value in the tables is one printed by a script in this pass or read from `module14.html:214`. `check_links` reports 124 internal links, 0 broken, 0 unlinked after the tables were added. |
...
| B8c | 280 (code) | The §6 code block's own print label changed from `peak force` to `mean force`, so the block stops contradicting the box it illustrates. | Block extracted from the edited file and run: prints `d=0.05 m -> mean force 6.2 kN` and `d=0.20 m -> mean force 2.1 kN`. `check_code`: 9 blocks, 0 pycodestyle issues. |
| B8d | 231 (Fig. 5) | Fig. 5's rotated y-axis title changed from `peak force (kN)` to `mean force (kN)`. The axis was the last place still asserting the old name, and it is the first thing a reader believes. | "peak" and "mean" are both four characters, so no glyph moves; `check_overlap` 0 and `check_frame` 0 re-run after the change. |
| B8e | 231 (Fig. 5) | Fig. 5's caption rewritten: "Peak landing force" to "Mean landing force"; the derivation re-attributed from **impulse-momentum** (which gives $mv/t$, not this) to **work-energy**, which is what Model 6.1 does; the peak-to-mean ratio named; and the two vague endpoints ("several kilonewtons", "about a third") replaced by the section's own $6.2$ and $2.1$ kN. | Both numbers are the §6 block's printed output. The curve itself was decoded from its own ticks (`m17/decode.py`): 100 drawn points against $F(d)=mv^2/2d+mg$ at $m=70$, $h=0.4$, maximum deviation **0.016 kN** over $d=0.02$ to $0.30$ m, so the drawing is the model and only the words were wrong. |
| B8f | 235 | §6's interpretation opens "averages $6.2\ \mathrm{kN}$ over the stroke" instead of "spikes to $6.2\ \mathrm{kN}$". A spike is a transient; this number is a stroke mean. | Same block output; the sentence three lines later already said the true peak exceeds this mean, which is what it contradicted. |
| B8g | 235 | §6's validation clause: "compare peak force and timing to instrumented drop landings" told the reader to test a mean against a measured peak. It now compares the mean over the stroke and the stroke duration, and checks the measured peak against the mean times the profile ratio. | Follows from B8a's mean/peak distinction; no number involved. |
| B8h | 326 (D2) | D2's statement asked the reader to derive $F_{\rm peak}\approx mgh/d+mg$; the box and D2's own solution both call that quantity a mean. Now derives $F_{\rm avg}$ and asks why the instantaneous peak exceeds it. | The solution shipped by B8b already derives $F_{\rm avg}$; grep confirms $F_{\rm peak}$ now appears only where a peak is meant (§5's running peak, K2, the parameter table). |
| B8i | 354 (diagnostic 4) | Diagnostic 4 asked how doubling the flexion distance changes "the peak force"; changed to "the mean landing force", matching its own solution, which reasons about the $+mg$ floor of the mean. | Same grep; S3 had already fixed the solution's physics, not its question. |
| B9a | 168 | Model 3.1's boxed equation: the knee inertia becomes $I_{\rm k}$, so the box no longer shares a symbol with the ankle inertia $I=mL^2$ of §2. | `checktex` 0 issues over 514 math segments; the §3 block re-run prints `peak knee torque = 109 N m (1.55 N m/kg)` unchanged. |
| B9b | 170 | The moment arm stops being written as the *function* $d(q)$ and is written $d_0\sin q$, removing the last bare $d$ from §3 (§6's $d$ is the landing distance). | `grep -c 'd(q)'` on the edited file returns 0; the only $d$ left in §3 is the subscripted $d_0$. |
| B9c | 170 | The definition beside the box now reads $I_{\rm k}$ and says why: "subscripted to keep it distinct from the whole-body $I=mL^2$ about the ankle in Section 2, which is a different quantity about a different axis", with a working link. | `check_links`: 124 internal links, 0 broken, 0 unlinked - unchanged, because B9i removed one link from the notation table as this added one. |
| B9d | 172 | §3's assumption line: same $d(q)$ to $d_0\sin q$ change. | Same grep. |
| B9e | 172 | §3's parameter line: $I=0.12\ \mathrm{kg\,m^2}$ becomes $I_{\rm k}=0.12\ \mathrm{kg\,m^2}$. | Value unchanged; the §3 block prints the same three-way split, `gravity 94.2, inertia 0.48, baseline 14.1 N m`. |
| B9f | 174-183 (code) | §3's code variable `Iz` renamed `I_k` to match the symbol the reader was just given. | Re-run: identical stdout. `check_code` 0 issues. |
| B9g | 174-183 (code) | §3's code: `iner = I_k*np.abs(qdd)`, and the comment `# d(q) = d0 sin q` becomes `# moment arm d0 sin q`. | Same run. |
| B9h | 347 (K4) | K4's solution reads the error budget off the symbol ("barely at all by $I$"); now $I_{\rm k}$, so the sentence points at the quantity it means. | K4's block re-run: `mean 108.6 N m, 1 sigma 15.3 N m (14.1%)`, `95% interval 80 to 140 N m`. |
| B9i | after 362 (notation) | The notation table's $I$ row was the flag that stood in for the fix: one row recording two values about two axes. It becomes two rows, $I$ (ankle, $mL^2=70$) and $I_{\rm k}$ (knee, $0.12$), neither carrying two meanings. The notation table is now 20 rows. | Counted in the edited file; `check_links` unchanged at 124/0/0. |
| B9j | after 362 (notation) | The $d_0$ row drops the $d(q)$ function form and states the arm in words: "at knee angle $q$ the arm is $d_0\sin q$". | Same grep as B9b. |
| B9k | after 362 (parameters) | The parameter table's "$I$ (about the knee)" row becomes "$I_{\rm k}$ (about the knee)". | Table read against the §3 parameter line; both now say $I_{\rm k}=0.12$. |
| B9l | 347 (K4 code) | K4's Monte Carlo block: `d` becomes `d0` and `Ii` becomes `I_k`, in the draws and in both uses inside the comprehension, so the code's names are the module's symbols. | Re-run: byte-identical stdout (`rng.default_rng(17)` is seeded, so this is an exact check, not a tolerance). Comment columns preserved; `check_code` 0 issues. |
| S17a | 347 (K4) | K4's solution illustrated the mislabelling trap with "$\pm15\%$", a number S5 removed from the module. Changed to the module's own one-sigma figure, $\pm14\%$. | The K4 block prints `1 sigma 15.3 N m (14.1%)`; $\pm14\%$ is what the module now quotes at both other places (`:225`, `:350`). |
| S17b | 347 (K4) | Same sentence's closing clause, "an unlabelled $\pm15\%$", to $\pm14\%$. | Same run; grep confirms no $\pm15\%$ remains in the module. |
